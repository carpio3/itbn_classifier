...
#############################
	...
	bagfile = os.environ["HOME"] + "/Documents/AssistiveRobotics/AutismAssistant/pomdpData/test_03/zga1.bag"
	timefile = os.environ["HOME"] + "/Documents/AssistiveRobotics/AutismAssistant/timing_labels/test_03/zga1.txt"
...
#############################
	...
	if(gen_single_file):
		...
		gen_TFRecord_from_file(out_dir=outdir, out_filename="scrap", bag_filename=bagfile, timing_filename=timefile, flip=False)
	...
	if(view_single_file):
		...
		with tf.Session() as sess:
			...
			for i in range(1):
				...
				l, p, a, tl, tv, n = sess.run([seq_len, opt_raw, aud_raw, timing_labels, timing_values, name])
			...
			def show(data, d_type):
				...
				if(data.shape[0] % 10 != 0):
					...
					fill = np.zeros((limit_size, limit_size*(10 - (data.shape[0] % 10)), d_type["num_c"]))#.fill(255)
					fill.fill(255)
...
#states = ["compliant", "noncompliant"]
	...
	if(process_all_files):
		path = os.environ["HOME"]+'/'+"Documents/AssistiveRobotics/AutismAssistant/pomdpData/long_sess_0"
		outdir = os.environ["HOME"]+'/'+"catkin_ws/src/deep_q_network/tfrecords/long/"
		for i in range(1,2):
			file_dir = path + str(i)
			filename_list = [file_dir+'/'+f for f in os.listdir(file_dir) if isfile(join(file_dir, f))]
			filename_list.sort()
			for f in filename_list:
				...
				gen_TFRecord_from_file(outdir=outdir, bagfile=f, state=state, name=new_name, flip=False, index=index)
				#writer.close()
