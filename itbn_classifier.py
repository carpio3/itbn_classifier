...
class ClassifierModel:
	...
	batch_size - int
			(1 by default)
	filename - string, location of file with saved model parameters 
			(no model listed by default)
	name - string, model name 
			("dqn" by default)
	learning_rate - float, speed at which the model trains
			(1e-5 by default)
	log_dir - directory to store summary information
			(no directory listed by default)
	'''
	def __init__(self, batch_size=1, win_size=10, filename="", 
					name="classifier", learning_rate=1e-5, log_dir=""):
		...
		self.variables_aud = {
			...
			"W_lstm" : weight_variable("W_lstm", [aud_layer_elements[-2],aud_layer_elements[-1]]),
			"b_lstm" : bias_variable("b_lstm", [aud_layer_elements[-1]]),
			"W_fc" : weight_variable("W_fc", [aud_layer_elements[-1]+1,aud_layer_elements[-1]]),
		...
		self.cross_entropy_wave =  tf.nn.softmax_cross_entropy_with_logits(labels=self.pnt_y_ph, logits=self.execute_wave())

		# optimize the network
		self.optimizer_wave = tf.train.AdamOptimizer(learning_rate=self.__alpha).minimize(self.cross_entropy_wave)


		# get the difference between the q-values and the true output
		self.cross_entropy_aud =  tf.nn.softmax_cross_entropy_with_logits(labels=self.aud_y_ph, logits=self.execute_aud())

		# optimize the network
		self.optimizer_aud = tf.train.AdamOptimizer(learning_rate=self.__alpha).minimize(self.cross_entropy_aud)
		...
		self.correct_pred_wave = tf.equal(tf.argmax(self.wave_observed,1), tf.argmax(self.pnt_y_ph,1))
		...
		# return a boolean indicating whether the system correctly predicted the output
		self.correct_pred_aud = tf.equal(tf.argmax(self.aud_observed,1), tf.argmax(self.aud_y_ph,1))
...
#######################
	...
	def wave_model(self, seq_length, pnt_ph, train_ph, prompts_ph,
		 variable_scope, variable_scope2, var_pnt, var_lstm=True):
		'''
		The DQN model
		...
		with variable_scope as scope:
			...
			conv_inp = tf.reshape(conv_inp, [self.__batch_size, -1, output_sizes[-1]*output_sizes[-1]*layer_elements[-2]], name="combine_reshape")
	...
	def aud_model(self, seq_length, aud_ph, train_ph, prompts_ph,
		...
		conv_inp = tf.reshape(conv_inp, [self.__batch_size, -1, aud_output_sizes[-1][0]*aud_output_sizes[-1][0]*aud_layer_elements[-2]], name="combine_reshape_aud")
