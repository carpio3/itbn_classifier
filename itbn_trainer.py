...
# file io
from itbn_tfrecord_rw import parse_sequence_example
...
def chunk_data(FRAME_SIZE, STRIDE, seq_len, timing_dict, opt_raw, aud_raw):
	...
	for i in range(num_chunks):
		opt_label_instance, aud_label_instance = label_data(FRAME_SIZE, STRIDE, i, seq_len, timing_dict)
...
if __name__ == '__main__':
	...
	dqn_chkpnt = ""
	dqn = ClassifierModel(batch_size=BATCH_SIZE, win_size=FRAME_SIZE, learning_rate=ALPHA, filename=dqn_chkpnt,\
				log_dir="LOG_DIR")
	...
	seq_len_inp, opt_raw_inp, aud_raw_inp, timing_labels_inp, timing_values_inp, name_inp = input_pipeline(filenames)
	...
	for iteration in range(NUM_ITER):
		...
		for i in range(num_chunks):
			if(i % 20 == 0):
				print("Processing chunk: "+str(i)+"/"+str(num_chunks[0])+" of file: "+name[0] )
			#---------------------------------------
			# Label Data
			#---------------------------------------

			opt_label_data, aud_label_data = label_data(FRAME_SIZE, STRIDE, i, seq_len, timing_dict)#one hot representation of the state or action
			...
			# optimize network
			_ = dqn.sess.run([dqn.optimizer_wave, dqn.optimizer_aud], feed_dict=vals, options=run_options, 
						run_metadata=run_metadata)
			
			opt_t += datetime.now() - opt_s
			
		'''
		opt_raw, aud_raw, opt_label_data, aud_label_data = chunk_data(FRAME_SIZE, STRIDE, seq_len, timing_dict, opt_raw, aud_raw )

		for chunk in range(opt_raw.shape[0]):
		...
		if(iteration%1 == 0):
			...
			print(iteration, "total_time:", str(datetime.now()-ts_it), "prep_time:",str(prep_t), "optimization_time:",str(opt_t))
		
		if(iteration%100 == 0):
			# evaluate system accuracy on train dataset
			wave_pred, aud_pred = dqn.sess.run([dqn.wave_observed, dqn.aud_observed], feed_dict=vals)
			'''
			print("pred: ", pred)
		...
		if(iteration % 1000 == 0):
			...
			dir_name = TAG+"_"+str(iteration / 10000)
